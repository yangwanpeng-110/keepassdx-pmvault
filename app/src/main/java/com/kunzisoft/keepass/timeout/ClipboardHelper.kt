...
/** Maps a clipboard label to the fixed PmVault field-type enumeration. */
private fun pmpAuditField(label: String, sensitive: Boolean): Int {
    val l = label.lowercase()
    return when {
        "totp" in l || "otp" in l || "验证码" in label ->
            com.kunzisoft.keepass.pmp.PmpAuditLog.FLD_TOTP
        "url" in l || "website" in l || "web site" in l || "网址" in label || "网站" in label ->
            com.kunzisoft.keepass.pmp.PmpAuditLog.FLD_URL
        "user" in l || "login" in l || "account" in l || "用户" in label || "账号" in label ->
            com.kunzisoft.keepass.pmp.PmpAuditLog.FLD_USERNAME
        "note" in l || "备注" in label ->
            com.kunzisoft.keepass.pmp.PmpAuditLog.FLD_NOTES
        sensitive || "pass" in l || "pwd" in l || "密码" in label ->
            com.kunzisoft.keepass.pmp.PmpAuditLog.FLD_PASSWORD
        else -> com.kunzisoft.keepass.pmp.PmpAuditLog.FLD_OTHER
    }
}
